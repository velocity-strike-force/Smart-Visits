...
export default function PostVisit() {
  ...
  const [customerMetadata, setCustomerMetadata] = useState<{arr?: number; status?: string; isKeyAccount?: boolean} | null>(null);

  const productLineOptions = ['Oracle Cloud', 'NetSuite', 'Shipping', 'TMS', 'Demand Planning', 'AX'];
  const domainOptions = ['Manufacturing', 'Technology', 'Logistics', 'Retail', 'Healthcare'];
  const purposeOptions = ['Quarterly Review', 'Product Demo', 'Training', 'Implementation Review', 'Contract Renewal', 'Other'];

  const mockCustomers = [
    { name: 'Acme Corp', arr: 250000, status: 'Active', isKeyAccount: true },
    { name: 'TechStart Inc', arr: 150000, status: 'Implementation', isKeyAccount: false },
    { name: 'Global Logistics', arr: 500000, status: 'Active', isKeyAccount: true },
    { name: 'RetailMax', arr: 180000, status: 'Active', isKeyAccount: false },
  ];
  ...
  const addInvitee = () => {
    if (inviteeInput.trim() && !formData.invitees.includes(inviteeInput.trim())) {
      setFormData({
        ...formData,
      ...
      <div className="max-w-4xl mx-auto p-8">
        <div className="bg-white rounded-lg border p-6 space-y-6">
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block mb-2 text-sm">Product Line</label>
              <select
            ...
            <div>
              <label className="block mb-2 text-sm">Sales Rep Name</label>
              <input
                type="text"
          ...
          <div className="relative">
            ...
            <input
              ...
              className="w-full px-3 py-2 border rounded-lg"
            />
            {customerSearch && filteredCustomers.length > 0 && !customerMetadata && (
            ...
            {customerMetadata && (
              <div className="flex gap-2 mt-2">
                <span className="px-2 py-1 bg-green-100 text-green-800 rounded text-xs">
                  ARR: ${customerMetadata.arr?.toLocaleString()}
                </span>
          ...
          <div>
            <label className="block mb-2 text-sm">Capacity (Max Attendees)</label>
            <input
              type="number"
              placeholder="e.g., 10"
          ...
          <div>
            ...
            <div className="flex flex-wrap gap-2">
              {formData.invitees.map(invitee => (
                <span key={invitee} className="px-3 py-1 bg-blue-100 text-blue-800 rounded-full text-sm flex items-center gap-2">
                  {invitee}
                  <button onClick={() => removeInvitee(invitee)} className="hover:text-blue-900">×</button>
                </span>
              ))}
            </div>
          </div>
          ...
          <div>
            <label className="block mb-2 text-sm">Customer Contact Rep</label>
            <input
              type="text"
          ...
          <div>
            <label className="block mb-2 text-sm">Purpose for Visit</label>
            <select
          ...
          <div>
            <label className="block mb-2 text-sm">Visit Details / Requirements</label>
            <textarea
          ...
          <div>
            <label className="block mb-2 text-sm">Key areas of focus</label>
            <textarea
              placeholder="What key areas were discussed during the visit?"
              value={formData.keyAreasOfFocus}
              onChange={(e) => setFormData({ ...formData, keyAreasOfFocus: e.target.value })}
              rows={3}
              className="w-full px-3 py-2 border rounded-lg"
            />
          </div>
          ...
          <div>
            ...
            <textarea
              ...
              onChange={(e) => setFormData({ ...formData, delighters: e.target.value })}
              rows={3}
              className="w-full px-3 py-2 border rounded-lg"
            />
          </div>
